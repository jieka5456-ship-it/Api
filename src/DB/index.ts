...
export async function insertOne(
  ...
  if (typeof data !== "object" || data === null || Array.isArray(data)) return JsonFail(400, "请求体必须是 JSON 对象");
  // 2) 找项目 schema
  ...
  if (missing.length) return JsonFail(400, `缺少必填字段: ${missing.join(", ")}`);

  // 5) 白名单过滤
  const keys = conf.insertable.filter((k) => obj[k] !== undefined);
  if (!keys.length) return JsonFail(400, "没有可插入字段");
  ...
  try {
    ...
    if (msg.includes("UNIQUE") || msg.includes("constraint failed")) {
      return JsonFail(400, "卡密已存在，不能重复添加");
    }

    return JsonFail(500, msg || "服务器内部错误");
  }
}
